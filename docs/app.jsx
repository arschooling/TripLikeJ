...
const CAT_PASTEL = ['#C96B5A','#D4975A','#C4A83A','#6EA87A','#4EA8A3','#5E8FC0','#8A70B5','#C4708A'];
const catColor = (ci) => CAT_PASTEL[ci % CAT_PASTEL.length];
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ padding:'10px 18px 4px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <button onClick={onClose} style={{
            ...
            fontFamily:SANS, fontSize:17, color:COLORS.mute, padding:'4px 2px',
          }}>취소</button>
          <div style={{ fontFamily:SERIF, fontSize:17, color:COLORS.ink }}>{title}</div>
          <button onClick={onConfirm} style={{
            border:'none', background:'transparent', cursor:'pointer',
            fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.accent, padding:'4px 2px',
          }}>{confirmLabel}</button>
        </div>
        {children}
      </div>
    ...
    return (
      <>
        <div style={{ padding:'0 14px 2px', display:'grid', gridTemplateColumns:'repeat(7, 1fr)' }}>
          {DOW.map((w, i) => (
            <div key={i} style={{ textAlign:'center', fontFamily:MONO, fontSize:16, letterSpacing:'0.08em',
            ...
            return (
              <button key={i} disabled={disabled}
                ...
                style={{ aspectRatio:'1/1', border:'none', cursor:disabled?'default':'pointer',
                  ...
                  fontFamily:SANS, fontSize:17, fontWeight:isSel?600:400,
        ...
        <div>
          ...
          <div style={{ padding:'0 16px 14px' }}>
            <button onClick={() => { setView({ y:+tmpY, mo:MONTH_KR.indexOf(tmpMo) }); setPickingYM(false); }} style={{
              ...
              fontFamily:SANS, fontSize:17, fontWeight:500, cursor:'pointer',
            }}>이 달 달력 보기</button>
          </div>
        </div>
      ) : (
...
function TimeField({ value, onChange }) {
  ...
  if (!open) return (
    <button type="button" onClick={() => setOpen(true)} style={{
      ...
      fontFamily:MONO, fontSize:17, color:COLORS.ink, cursor:'pointer',
      display:'flex', alignItems:'center', justifyContent:'center',
    }}>
      {value || '09:00'}
    </button>
    ...
    <BottomSheet open={open}
        ...
        {['start','end'].map((k,ki) => {
          ...
          return (
            <button key={k} onClick={()=>{ setPicking(k); setShowYM(false); }} style={{
              ...
              <div style={{fontFamily:MONO, fontSize:16, letterSpacing:'0.12em', marginBottom:3,
      ...
      <div style={{display:'flex', alignItems:'center', padding:'0 16px', marginBottom:10, gap:8}}>
        ...
        <button onClick={() => {
          ...
          <span style={{fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.ink, letterSpacing:'-0.01em'}}>
      ...
      {showYM ? (
        ...
        <>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(7,1fr)', padding:'0 14px 2px'}}>
            {DOW.map((w,i) => (
              <div key={i} style={{textAlign:'center', fontFamily:MONO, fontSize:16, letterSpacing:'0.08em', padding:'2px 0 6px',
            ...
            {cells.map((d,i) => {
              ...
              return (
                <button key={i} onClick={()=>handleDay(d)} style={{
                  ...
                  <span style={{position:'relative',zIndex:1,fontFamily:SANS,fontSize:17,fontWeight:isSel?600:400,
      ...
      <div onClick={e => e.stopPropagation()}
          ...
          <div style={{ background:COLORS.card, borderRadius:10, padding:'10px 12px',
            ...
            <input ref={inputRef} value={q} onChange={e => setQ(e.target.value)} placeholder="검색"
              style={{ border:'none', outline:'none', background:'transparent', flex:1,
                fontFamily:SANS, fontSize:17, color:COLORS.ink }}/>
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'0 16px calc(80px + env(safe-area-inset-bottom,0px))' }}>
          <div style={{ background:COLORS.card, borderRadius:14, overflow:'hidden' }}>
            {filtered.length === 0 && (
              <div style={{ padding:'20px', fontFamily:SANS, fontSize:17, color:COLORS.mute, textAlign:'center' }}>검색 결과 없음</div>
            )}
            {filtered.map((item, i) => {
              const k = getKey(item);
...
const CITIES = [
  ...
  { key:'Shanghai',    kor:'상하이',     zone:'Asia/Shanghai',       flag:'🇨🇳', lat:31.23,  lon:121.47,  currency:'USD' },
...
function detectCurrencyFromTitle(title) {
  ...
  if (city?.currency && ['EUR','JPY','PHP'].includes(city.currency)) return city.currency;
  const t = title.toLowerCase();
  if (/japan|일본|osaka|교토|오사카|나고야/.test(t)) return 'JPY';
  if (/europe|유럽|france|프랑스|italy|이탈리아|florence|피렌체|venice|베네치아|milan|밀라노|germany|독일|spain|스페인|barcelona|마드리드|madrid|netherlands|amsterdam|portugal|lisbon|리스본|greece|athens|아테네|vienna|빈|switzerland|취리히|prague|프라하|budapest|부다페스트/.test(t)) return 'EUR';
  if (/philippines|필리핀|manila|마닐라|cebu|세부|boracay|보라카이/.test(t)) return 'PHP';
  return 'USD';
}
  ...
  const ModeTab = ({ id, emoji, label, desc }) => {
    ...
    return (
      <div onClick={() => setMode(id)} style={{
        ...
        <div style={{ fontFamily:SANS, fontSize:16, color: active ? 'rgba(255,255,255,0.65)' : COLORS.mute,
    ...
    <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(0,0,0,0.4)',
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ flexShrink:0 }}>
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.accent, letterSpacing:'0.14em', marginBottom:4 }}>SHARE TRIP</div>
        ...
        <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch' }}>
          {loading ? (
            <div style={{ textAlign:'center', padding:'20px 0', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>불러오는 중...</div>
          ) : mode === 'edit' ? (
            <>
              {memberProfiles.length > 0 && (
                <>
                  <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, letterSpacing:'0.08em',
                    ...
                    <div key={c.uid} style={{
                      ...
                      {c.photoURL
                        ...
                        : <div style={{ width:38, height:38, borderRadius:'50%', background:'#C8C3B8', flexShrink:0,
                            display:'flex', alignItems:'center', justifyContent:'center',
                            fontFamily:SANS, fontSize:17, color:'#fff', fontWeight:600 }}>
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        ...
                        <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute,
              ...
              {msg && (
                <div style={{ marginTop:8, fontFamily:SANS, fontSize:17,
            ...
            <>
              <div style={{ background:COLORS.softer, borderRadius:14, padding:'12px 14px', marginBottom:16 }}>
                <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.ink, lineHeight:1.5 }}>
        ...
        <button onClick={handleSend} disabled={sending || !canSend} style={{
          ...
          fontFamily:SANS, fontSize:17, fontWeight:500, cursor: canSend ? 'pointer' : 'default',
  ...
  return (
    <div style={{ minHeight:'100vh', background:COLORS.bg, paddingBottom:90, position:'relative' }}>
      ...
      <div style={{
        ...
        <div style={{ fontFamily:SERIF, fontSize:34, color:COLORS.ink, letterSpacing:'-0.02em' }}>My Trips<span style={{fontFamily:'monospace',fontSize:12,color:COLORS.mute,marginLeft:8}}>v219</span></div>
      </div>
      {loading && trips.length === 0
        ...
        : <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <button onClick={onAdd} style={{ marginTop:4, padding:'18px 16px', background:'transparent',
              ...
              display:'flex', gap:8, alignItems:'center', justifyContent:'center', fontFamily:SANS, fontSize:17 }}>
              {t('addTrip')}
            </button>
          </div>
      }
      <div style={{ textAlign:'center', paddingTop:20 }}>
        <div className="arshooling-text" style={{ fontFamily:'Adam, serif', fontSize:17, letterSpacing:'0.18em', color:COLORS.mute }}>ARSHOOLING</div>
      </div>
    </div>
  );
}
...
// ─── Ticket Viewer ──────────────────────────────────────────
function TicketViewer({ ticket, onClose, onEdit, onDelete }) {
    ...
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:90, position:'relative' }}>
      ...
      {onBack && (
        <button onClick={onBack} style={{
          ...
          display:'flex', alignItems:'center', gap:3,
          fontFamily:SANS, fontSize:17, color:COLORS.mute,
        }}>
      ...
      <div style={{ padding:'10px 24px 18px' }}>
        ...
        <div style={{ marginTop:10, display:'flex', gap:6, alignItems:'center', flexWrap:'wrap' }}>
          {editing ? (
            ...
            <span style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute }}>{trip.dates}</span>
          )}
          <span style={{ color:COLORS.mute, opacity:0.4, fontSize:17 }}>·</span>
          <span style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute }}>{trip.days.length} days</span>
        </div>
        {/* 색깔 선택 (수정 모드) */}
        {editing && (
                ...
                <div style={{
                  ...
                  <div style={{ padding:'16px 18px 18px' }}>
                    ...
                    <button onClick={() => !fGesture.current.drag && onOpenDay(i)} style={{
                      ...
                      padding:'13px 16px', fontFamily:SANS, fontSize:17, fontWeight:500,
      ...
      {(() => {
        ...
        return (
          <>
            ...
            <div style={{ padding:'0 16px', display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:10, alignItems:'start' }}>
              {tickets.map((card) => {
                const cardFiles = getTicketFiles(card);
                const firstFile = cardFiles[0];
                const isImg = firstFile?.type?.startsWith('image/');
                return (
                  <div key={card.id} onClick={() => setTicketViewer(card)} style={{ cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'stretch', gap:6, minWidth:0 }}>
                    {/* Square icon thumbnail — paddingBottom trick guarantees square */}
                    <div style={{ width:'100%', paddingBottom:'100%', position:'relative', background:COLORS.softer, borderRadius:14, overflow:'hidden' }}>
                      {isImg ? (
                        <img src={firstFile.url} style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover' }} alt=""/>
                      ) : (
      ...
      {ticketViewer && ReactDOM.createPortal(<TicketViewer
        ticket={ticketViewer}
        onClose={() => setTicketViewer(null)}
        onEdit={() => { setTicketEditCardId(ticketViewer.id); setTicketEditName(ticketViewer.name || ''); setTicketViewer(null); }}
        onDelete={() => { handleTicketDeleteCard(ticketViewer); setTicketViewer(null); }}
      />, document.body)}
      {ticketEditCardId && (() => {
          ...
          <div onClick={() => setTicketEditCardId(null)} style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                ...
                <div style={{ fontFamily:SERIF, fontSize:17, color:COLORS.ink }}>바우처 수정</div>
              ...
              <button onClick={handleEditAddFile} disabled={isAdding} style={{
                ...
                display:'flex', gap:8, alignItems:'center', justifyContent:'center',
                fontFamily:SANS, fontSize:17, color:COLORS.mute, width:'100%',
  ...
  return (
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
      <div style={{ position:'relative', background: heroBg }}>
        {dayPhoto
          ? <img src={dayPhoto} alt="" style={{ width:'100%', height:'calc(280px + env(safe-area-inset-top, 0px))', objectFit:'cover', display:'block' }}/>
            ...
            {editing ? (
              <div style={{ marginTop:8, display:'flex', flexDirection:'column', gap:6 }}>
                ...
                <input value={day.titleEn || ''} onChange={e => onEditDay({ titleEn: e.target.value })}
                  ...
                  style={{ width:'100%', padding:'8px 10px', borderRadius:8, boxSizing:'border-box',
                    border:`1px solid ${COLORS.line}`, background:COLORS.card,
                    fontFamily:SANS, fontSize:17, fontStyle:'italic', color:COLORS.mute, outline:'none' }}/>
              </div>
            ) : (
              <>
                ...
                {day.titleEn && (
                  <div style={{ marginTop:2, fontFamily:SANS, fontSize:17, color:COLORS.mute, fontStyle:'italic' }}>
                    {day.titleEn}
                  </div>
                )}
              </>
                    ...
                    {editing && inlineItemTitle?.idx === i ? (
                      <input autoFocus value={inlineItemTitle.title}
                        ...
                        style={{ width:'100%', marginTop:3, padding:'2px 0', border:'none',
                          ...
                          fontFamily:SANS, fontSize:17, fontWeight:500, color:COLORS.ink,
                          outline:'none', boxSizing:'border-box' }}/>
                    ) : (
                      <div
                        onClick={editing ? (e) => { e.stopPropagation(); setInlineItemTitle({ idx: i, title: stopTitle(it, t) }); } : undefined}
                        style={{ marginTop:3, fontFamily:SANS, fontSize:17, fontWeight:500,
        ...
        {(isDate || isTime) ? (
          <button onClick={() => setPickerOpen({ key, type })}
            style={{ width:'100%', padding:'10px 12px', borderRadius:8,
              ...
              fontFamily:SANS, fontSize:17, color: shown ? COLORS.ink : COLORS.mute,
          ...
          <input value={draft[key] || ''} onChange={e => setDraft({...draft, [key]: e.target.value})}
            ...
            style={{ width:'100%', padding:'9px 11px', borderRadius:8, border:`1px solid ${COLORS.line}`,
              background:COLORS.card, fontFamily:SANS, fontSize:17, color:COLORS.ink, boxSizing:'border-box' }}/>
        )}
      </div>
    );
  };
  ...
  return (
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
      ...
      <div style={{ padding:'22px 20px 8px' }}>
        ...
        {draft.area && (
          <div style={{ marginTop:4, fontFamily:SANS, fontSize:17, color:COLORS.mute, display:'flex', gap:5, alignItems:'center' }}>
          ...
          <div style={{ background:COLORS.card, borderRadius:14, padding:'14px 16px' }}>
            ...
            <div style={{ marginBottom:10 }}>
              ...
              <textarea value={draft.note || ''} onChange={e => setDraft({...draft, note: e.target.value})}
                rows={3}
                style={{ width:'100%', padding:'9px 11px', borderRadius:8, border:`1px solid ${COLORS.line}`,
                  background:COLORS.card, fontFamily:SANS, fontSize:17, color:COLORS.ink,
          ...
          {draft.note && (
            <div style={{ padding:'16px 16px 0' }}>
              <div style={{ background:COLORS.softer, borderRadius:12, padding:'14px 16px',
                fontFamily:SANS, fontSize:17, color:COLORS.mute, lineHeight:1.5 }}>
                {draft.note}
              </div>
            </div>
          )}
    ...
    return (
      <button key={item.name} onClick={() => window.open(mapsSearchUrl(item.name), '_blank')}
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {item.nameOrig && <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:1, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{item.nameOrig}</div>}
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, marginTop:3, display:'flex', gap:6, alignItems:'center' }}>
  ...
  return ReactDOM.createPortal(
    <div style={{ position:'fixed', inset:0, zIndex:1100,
      ...
      <div ref={sheetRef} onClick={e=>e.stopPropagation()}
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            <button onClick={() => setSort(s => s === 'relevance' ? 'distance' : 'relevance')}
              style={{ border:'none', background:COLORS.softer, borderRadius:10, padding:'6px 10px',
                fontFamily:MONO, fontSize:16, cursor:'pointer', letterSpacing:'0.04em',
        ...
        {/* 목록 */}
        {loading && (
          <div style={{ padding:'40px 20px', textAlign:'center', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>
            주변 장소를 찾는 중...
          </div>
        )}
    ...
    <div style={{ position:'fixed', inset:0, zIndex:1000,
      ...
      <div ref={sheetRef} onClick={(e)=>e.stopPropagation()}
          ...
          <div style={{ display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            {!editing && (
              <button onClick={(e) => { e.stopPropagation(); setEditing(true); }} style={{
                ...
                fontFamily:SANS, fontSize:16, fontWeight:500, color:COLORS.ink,
        ...
        <div style={{ padding:'18px 20px 0' }}>
          <div style={{ display:'flex', gap:6, alignItems:'center',
            fontFamily:MONO, fontSize:16, color:COLORS.mute,
            letterSpacing:'0.12em', textTransform:'uppercase' }}>
          ...
          {editing ? (
            ...
            <>
              ...
              {draft.en && (
                <div style={{ marginTop:4, fontFamily:SANS, fontSize:17, fontStyle:'italic', color:COLORS.mute }}>
                  {draft.en}
                </div>
              )}
              <div style={{ marginTop:16, display:'flex', flexDirection:'column' }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute,
                        letterSpacing:'0.12em', textTransform:'uppercase' }}>{r.label}</div>
                      <div style={{ marginTop:2, fontFamily:SANS, fontSize:17, color:COLORS.ink,
...
function HotelSheet({ open, onClose, hotel, trip, tripDays, onSave, onDelete, onRegisterEdit, onTabBarToggle }) {
  ...
  const field = (key, label, placeholder = '') => (
    <div style={{ marginBottom:10 }}>
      <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>{label}</div>
    ...
    <div style={{ position:'fixed', inset:0, zIndex:1000,
      ...
      <div style={{ transform:`translateY(${entered ? sheetY : window.innerHeight}px)`,
        ...
        <div ref={sheetRef} onClick={e => e.stopPropagation()}
          ...
          <div style={{ padding:'18px 20px 0' }}>
            <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.accent, letterSpacing:'0.12em', textTransform:'uppercase' }}>
            ...
            {editing ? (
              <div style={{ marginTop:14 }}>
                <div style={{ position:'relative', marginBottom:10 }}>
                  <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>{t('wizHotelPh')}</div>
                  <div style={{ position:'relative' }}>
                    <input
                      ...
                      style={{ width:'100%', padding:'9px 36px 9px 11px', borderRadius:8,
                        ...
                        fontFamily:SANS, fontSize:17, color:COLORS.ink, boxSizing:'border-box' }}/>
                      ...
                      {searchRes.map((f, i) => {
                        ...
                        return (
                          <button key={i}
                            ...
                            {city && <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:1 }}>{[addr, city].filter(Boolean).join(', ')}</div>}
                          </button>
                        );
                      })}
                    </div>
                ...
                <div style={{ background:COLORS.card, borderRadius:14, padding:'14px 16px' }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10 }}>
                    {['checkin','checkout'].map(k => (
                      <div key={k}>
                        <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>
                        ...
                        <button onClick={()=>setTimePicker(k)} style={{ width:'100%', padding:'9px 11px',
                          ...
                          fontFamily:SANS, fontSize:17, color:COLORS.ink, cursor:'pointer', textAlign:'left',
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                    <div>
                      <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>박수</div>
                  ...
                  <div>
                    <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:4 }}>메모</div>
              ...
              <div style={{ marginTop:16, display:'flex', flexDirection:'column' }}>
                {rows.map((r, i, arr) => (
                  <div key={i} style={{
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.12em', textTransform:'uppercase' }}>{r.label}</div>
                ...
                <>
                  ...
                  {!isNew && (
                    <button onClick={() => { setDraft({ ...committed.current }); setEditing(false); }} style={{
                      width:80, background:COLORS.card, border:`1px solid ${COLORS.line}`,
                      borderRadius:12, cursor:'pointer', fontFamily:SANS, fontSize:17, color:COLORS.ink,
                    }}>취소</button>
                  )}
                </>
              ) : (
        ...
        {timePicker && (
          <div onClick={() => setTimePicker(null)} style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ padding:'4px 20px 10px', fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', textTransform:'uppercase' }}>
                ...
                return (
                  <button key={t} onClick={() => { setDraft({...draft, [k]:t}); setTimePicker(null); }}
                    style={{ display:'flex', width:'100%', padding:'13px 20px', border:'none',
                      ...
                      fontFamily:SANS, fontSize:17, color:COLORS.ink, cursor:'pointer',
    ...
    <label style={{ display:'block', marginTop:10 }}>
      ...
      {type === 'textarea' ? (
        <textarea value={draft[key] || ''} onChange={e => setDraft({...draft, [key]: e.target.value})}
          ...
          border:`1px solid ${COLORS.line}`, background:COLORS.card,
          fontFamily:SANS, fontSize:17, color:COLORS.ink, boxSizing:'border-box', resize:'vertical' }}/>
      ) : type === 'select' ? (
    ...
    <div style={{ marginTop:10 }}>
      ...
      {draft.cat === 'hotel' && (
        <button type="button" onClick={() => setShowHotelSearch(true)} style={{
          ...
          padding:'11px', fontFamily:SANS, fontSize:17, fontWeight:500,
          display:'flex', gap:6, alignItems:'center', justifyContent:'center',
        }}>
...
function PlaceSearchSheet({ open, item, cityBias, onClose, onPick }) {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="장소 검색">
      ...
      {loading && (
        <div style={{ textAlign:'center', padding:'16px 0', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>검색 중...</div>
      )}
      ...
      <div style={{ padding:'0 8px 20px', display:'flex', flexDirection:'column', gap:2 }}>
          ...
          return (
            <button key={i} onClick={() => onPick({ name, addr, coords:[lat, lon] })} style={{
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontFamily:SANS, fontSize:17, fontWeight:500, color:COLORS.ink,
                  whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{name}</div>
                {addr ? <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:2,
        ...
        {!loading && query && results.length === 0 && (
          <div style={{ textAlign:'center', padding:'20px 0', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>결과 없음</div>
        )}
      </div>
    </BottomSheet>
  );
...
function MapScreen({ trip, onEditItem, editing, onRegisterEdit }) {
  ...
  return (
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
            ...
            {routeTip.isOptimal ? (
              <div>
                <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
                  <span style={{ fontSize:17 }}>🎉</span>
                  <span style={{ fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.ink }}>완벽한 동선이에요!</span>
                </div>
                <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute, lineHeight:1.6 }}>
                  {routeTip.hotel
              ...
              <div>
                <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.ink, marginBottom:6, lineHeight:1.55 }}>
                  {routeTip.hotel
      ...
      {openStop && ReactDOM.createPortal(
        <StopSheet
          open={openStop}
          dayHue={heroHue}
          cityBias={cityBias}
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:4, paddingRight:32 }}>
                    <input value={f.name} onChange={e => updateFood(f.idx, { name: e.target.value })}
                      ...
                      style={{ border:'none', outline:'none', background:'transparent',
                        fontFamily:SANS, fontSize:17, fontWeight:500, color:COLORS.ink, padding:0, width:'100%' }}/>
  ...
  return (
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
      ...
      <div style={{ padding:'0 16px 16px', position:'relative' }}>
        ...
        <input value={query} onChange={e => setQuery(e.target.value)}
          ...
          style={{ width:'100%', boxSizing:'border-box',
            ...
            background:COLORS.card, fontFamily:SANS, fontSize:17, color:COLORS.ink, outline:'none' }}/>
      </div>
      ...
      <div style={{ padding:'0 16px', display:'flex', flexDirection:'column', gap:20 }}>
              ...
              {addSelCat === cat ? (
                <div style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 14px',
                  ...
                  <button onClick={() => addFood(cat)} style={{
                    ...
                    background:COLORS.accent, color:'#fff', fontFamily:SANS, fontSize:17, fontWeight:600, cursor:'pointer' }}>
                    추가
                  </button>
                  <button onClick={() => setAddSelCat(null)} style={{
                    padding:'8px 12px', border:'none', borderRadius:10,
                    background:COLORS.softer, color:COLORS.mute, fontFamily:SANS, fontSize:17, cursor:'pointer' }}>
                    취소
                  </button>
                </div>
              ) : (
                <button onClick={() => setAddSelCat(cat)} style={{
                  ...
                  color:COLORS.mute, fontFamily:SANS, fontSize:17 }}>
        ...
        {/* 빈 상태 */}
        {totalFiltered === 0 && (
          <div style={{ padding:'32px 0', textAlign:'center', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>
          ...
          <form onSubmit={e => { e.preventDefault(); addCategory(); }}
            ...
            <input autoFocus value={newCatName} onChange={e => setNewCatName(e.target.value)}
              ...
              style={{ flex:1, padding:'11px 14px', border:`1px solid ${COLORS.line}`, borderRadius:12,
                fontFamily:SANS, fontSize:17, background:COLORS.card, color:COLORS.ink, outline:'none' }}/>
            <button type="submit" style={{ padding:'11px 16px', border:'none', borderRadius:12,
              background:COLORS.ink, color:'#fff', fontFamily:SANS, fontSize:17, fontWeight:600, cursor:'pointer' }}>추가</button>
            <button type="button" onClick={() => { setNewCatInput(false); setNewCatName(''); }}
              style={{ padding:'11px 12px', border:'none', borderRadius:12,
                background:COLORS.softer, color:COLORS.mute, fontFamily:SANS, fontSize:17, cursor:'pointer' }}>취소</button>
          </form>
        ) : (
            ...
            >
              <div style={{ display:'flex', alignItems:'center', gap:12,
                ...
                {(editing || isEditingThis) ? (
                  ...
                  <span style={{ flex:1, fontFamily:SANS, fontSize:17, color:COLORS.ink,
    ...
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
              ...
              {editing && renamingCat === ci ? (
                <input autoFocus value={cat.name}
                  ...
                  style={{ flex:1, border:`1px solid ${COLORS.line}`, borderRadius:8,
                    padding:'4px 8px', fontFamily:MONO, fontSize:17, letterSpacing:'0.12em',
                    textTransform:'uppercase', background:COLORS.card, color:catColor(ci), outline:'none', fontWeight:700 }}/>
              ) : (
              ...
              <div style={{ background:COLORS.card, borderRadius:14 }}>
                {(cat.items || []).length === 0 && addInputCat !== ci && (
                  <div ref={el => { prepItemEls.current[`${ci}_empty`] = el; }}
                    style={{ padding:'14px 16px', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>
                    항목이 없어요
                  </div>
                )}
                ...
                {addInputCat === ci ? (
                  <div style={{ display:'flex', alignItems:'center', gap:8, padding:'12px 14px',
                    ...
                    <button onClick={() => addItem(ci)} style={{
                      ...
                      background:COLORS.accent, color:'#fff', fontFamily:SANS, fontSize:17, cursor:'pointer' }}>추가</button>
                  </div>
                ) : (
      ...
      {copyToast && (
        <div style={{ position:'fixed', bottom:100, left:'50%', transform:'translateX(-50%)',
          ...
          fontFamily:SANS, fontSize:17, zIndex:2000, whiteSpace:'nowrap',
            ...
            <div style={{ padding:'0 16px', flex:1, overflowY:'auto', display:'flex', flexDirection:'column', gap:12 }}>
              <textarea
                ...
                style={{ width:'100%', boxSizing:'border-box', minHeight:280, border:`1px solid ${COLORS.line}`,
                  borderRadius:14, padding:'14px 16px', fontFamily:SANS, fontSize:17, color:COLORS.ink,
    ...
    <div style={{ position:'fixed', inset:0, zIndex:309,
      ...
      <div ref={sheetRef} onClick={e=>e.stopPropagation()} style={{
        ...
        <div style={{ padding:'8px 20px 14px', textAlign:'right', minHeight:64 }}>
          {op && <span style={{ fontFamily:MONO, fontSize:17, color:COLORS.mute, marginRight:8 }}>{op}</span>}
        ...
        <div style={{ padding:'12px 14px 16px', display:'flex', gap:8 }}>
          <button onClick={() => { onEnter('in', displayNum); onClose(); }} style={{
            ...
            background:COLORS.card, fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.ink, cursor:'pointer',
          }}>수입으로 입력</button>
          <button onClick={() => { onEnter('out', displayNum); onClose(); }} style={{
            flex:1, padding:'14px 0', border:'none', borderRadius:14,
            background:COLORS.accent, fontFamily:SANS, fontSize:17, fontWeight:600, color:'#fff', cursor:'pointer',
          }}>지출로 입력</button>
        </div>
      </div>
      </div>
...
function SplitSheet({ open, onClose, shared, splitN, onChangeSplitN, onTabBarToggle }) {
  ...
  const sharedCurs = Object.entries(shared || {}).filter(([,{out}]) => out > 0);
    ...
    <div style={{ position:'fixed', inset:0, zIndex:310,
      ...
      <div ref={sheetRef} onClick={e=>e.stopPropagation()} style={{
        ...
        {sharedCurs.map(([cur, {out}]) => {
          ...
          return (
            <div key={cur} style={{ background:COLORS.card, borderRadius:14, padding:'12px 16px', marginBottom:8 }}>
              <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6 }}>{cur} 공동 지출</div>
              <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
                <div style={{ fontFamily:SERIF, fontSize:18, color:COLORS.ink }}>{fmtAmt(out, cur)}</div>
                <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute }}>÷ {count} =</div>
        ...
        <div style={{ background:COLORS.card, borderRadius:14, padding:'14px 16px', marginBottom:14 }}>
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:8 }}>인원 수</div>
...
function BudgetImportExportSheet({ open, onClose, entries, myUid, onImport }) {
  ...
  const parseImport = () => {
    const raw = importText.trim().replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    ...
    if (isHeader) {
      ...
      firstCells.forEach((c, i) => {
        for (const [field, aliases] of Object.entries(FIELD_ALIASES)) {
          if (aliases.includes(c) && colIdx[field] === undefined) colIdx[field] = i;
        }
      });
    } else {
    ...
    for (let i = startIdx; i < lines.length; i++) {
      ...
      imported.push({
        ...
        type, amount,
        cat:      getCell(cells, 'cat') || '기타',
        scope, date,
        currency: getCell(cells, 'currency') || 'KRW',
        note:     getCell(cells, 'note'),
        createdBy: myUid,
      });
    }
  ...
  const tabBtnStyle = (t) => ({
    ...
    fontFamily:SANS, fontSize:17, fontWeight: tab===t ? 600 : 400,
      ...
      {tab === 'export' ? (
        <div style={{ padding:'0 16px 24px' }}>
          {entries.length === 0 ? (
            ...
            <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, marginBottom:8 }}>
            ...
            <textarea readOnly value={toTsv}
              ...
              style={{
                ...
                fontSize:16, fontFamily:MONO, color:COLORS.ink, background:COLORS.soft,
            ...
            <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:10, textAlign:'center' }}>
        ...
        <div style={{ padding:'0 16px 24px' }}>
          <div style={{ fontFamily:SANS, fontSize:16.5, color:COLORS.mute, marginBottom:10, lineHeight:1.6 }}>
          ...
          {entries.length > 0 && importText.trim() && (
            <div style={{ marginBottom:10, padding:'8px 12px', borderRadius:10,
              background:'rgba(200,80,80,0.08)', fontFamily:SANS, fontSize:16.5, color:COLORS.accent }}>
          ...
          <textarea
            ...
            style={{
              ...
              border:`1.5px solid ${COLORS.line}`, fontFamily:MONO, fontSize:16,
...
function BudgetScreen({ trip, myUid, onEditBudget, onSheetChange, onTabBarToggle }) {
  ...
  // visibleEntries 통계 - 통화 변환 없이 원본 금액 유지
  const budgetStats = React.useMemo(() => {
    const shared = {}, personal = {};
    let hasShared = false;
    visibleEntries.forEach(e => {
      const cur = e.currency || 'KRW';
      const isShared = (e.scope || 'personal') === 'shared';
      const target = isShared ? shared : personal;
      if (!target[cur]) target[cur] = { out: 0, in: 0 };
      if (e.type === 'out') target[cur].out += e.amount;
      else target[cur].in += e.amount;
      if (isShared) hasShared = true;
    });
    return { shared, personal, hasShared };
  }, [visibleEntries]);
  const { shared, personal, hasShared } = budgetStats;
  const hasSharedOut = Object.values(shared).some(v => v.out > 0);
  ...
  const openAdd = (type) => {
    ...
    setForm({ type, amount:'', cat: cats[0] || '', note:'', date:'', currency:'USD', scope:'personal' });
  ...
  const openAddWithAmount = (type, amount) => {
    const cats = type === 'out' ? outCats : inCats;
    setForm({ type, amount: String(Math.round(amount)), cat: cats[0] || '', note:'', date:'', currency:'USD', scope:'personal' });
    setEditIdx(null); setDelConfirm(false); setAddingCat(false); setNewCatVal('');
    setAddOpen(true);
    onSheetChange?.(true);
  };
    ...
    <div style={{ background:COLORS.bg, minHeight:'100vh', paddingBottom:110 }}>
      ...
      <div style={{ margin:'0 16px 14px', background:COLORS.panel, borderRadius:20, padding:'20px 20px 18px' }}>
        {hasShared ? (
          ...
          Object.keys(personal).length > 0 ? (
            ...
            <div style={{ fontFamily:SANS, fontSize:17, color:'rgba(255,255,255,0.3)', textAlign:'center', padding:'6px 0' }}>기록 없음</div>
          )
        )}
      </div>
      ...
      <div style={{ padding:'0 16px 16px', display:'flex', gap:8 }}>
        <button onClick={() => openAdd('in')} style={{
          ...
          fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.ink, cursor:'pointer',
        ...
        <button onClick={() => openAdd('out')} style={{
          ...
          fontFamily:SANS, fontSize:17, fontWeight:600, color:'#fff', cursor:'pointer',
          display:'flex', alignItems:'center', justifyContent:'center', gap:6,
        }}>
      ...
      {visibleEntries.length === 0 ? (
        <div style={{ padding:'60px 0', textAlign:'center' }}>
          ...
          <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute }}>여행 수입과 지출을 기록해 보세요</div>
        </div>
      ) : (
          <div style={{ padding:'0 16px' }}>
                  ...
                  {items.map((e, i) => {
                    ...
                    <SwipeableRow
                      ...
                      <div style={{
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.ink, marginBottom:2,
                            whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                            {e.note || e.cat}
                          </div>
                        ...
                        <div style={{ fontFamily:MONO, fontSize:17, fontWeight:600, flexShrink:0,
          ...
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <select value={form.currency||'KRW'} onChange={e => setForm(f => ({...f, currency: e.target.value}))}
              style={{ width:'100%', boxSizing:'border-box', padding:'11px 14px', marginBottom:12,
                border:`1px solid ${COLORS.line}`, borderRadius:12, background:COLORS.card,
                fontFamily:MONO, fontSize:17, color:COLORS.ink, outline:'none',
            ...
            <input value={form.note} onChange={e => setForm(f => ({...f, note:e.target.value}))}
              ...
              style={{ width:'100%', boxSizing:'border-box', padding:'11px 16px', marginBottom:10,
                border:`1px solid ${COLORS.line}`, borderRadius:12, background:COLORS.card,
                fontFamily:SANS, fontSize:17, color:COLORS.ink, outline:'none' }}/>
            {/* 날짜 */}
            <button type="button" onClick={() => setDatePickOpen(true)} style={{
              width:'100%', boxSizing:'border-box', padding:'11px 16px', marginBottom:16,
              border:`1px solid ${COLORS.line}`, borderRadius:12, background:COLORS.card,
              fontFamily:SANS, fontSize:17, color: form.date ? COLORS.ink : COLORS.mute,
            ...
            <div style={{ display:'flex', gap:8 }}>
              {editIdx !== null && !delConfirm && (
                <button onClick={() => setDelConfirm(true)} style={{
                  ...
                  fontFamily:SANS, fontSize:17, color:COLORS.mute, cursor:'pointer' }}>
                  삭제
                </button>
              )}
              {editIdx !== null && delConfirm && (
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize:17, color:COLORS.mute, letterSpacing:'0.04em', textAlign:'center',
      }}>
    ...
    <div style={{ minHeight:'100vh', background:COLORS.bg, display:'flex', flexDirection:'column',
      ...
      <button onClick={handleLogin} disabled={loading}
        style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 28px',
          ...
          fontFamily:SANS, fontSize:17, fontWeight:500, color:'#1A1816',
      ...
      {displayErr && (
        <div style={{ marginTop:20, padding:'12px 16px', background:'#FEF2F2', borderRadius:12,
          fontFamily:SANS, fontSize:17, color:'#C0392B', maxWidth:300, textAlign:'center', lineHeight:1.5 }}>
          {displayErr}
        </div>
      )}
    </div>
...
function NotificationsScreen({ open, onClose, authUser, notifications, onGoToCompanions, onGoToTrip }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {notifications.length > 0 && (
          <button type="button" onClick={deleteAllNotifs} style={{
            ...
            fontFamily:SANS, fontSize:17, color:COLORS.mute, flexShrink:0,
          }}>전체 삭제</button>
        )}
      </div>
      ...
      <div style={{ padding:'16px 16px 0' }}>
        {notifications.length === 0 ? (
          <div style={{ padding:'48px 0', textAlign:'center', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>
            알림이 없습니다
          </div>
        ) : (
      ...
      {addTripFor && (
        <div style={{ position:'fixed', inset:0, zIndex:300, background:'rgba(0,0,0,0.4)',
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute, marginBottom:16 }}>어느 여행에 추가할까요?</div>
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {(trips||[]).map(t => {
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        ...
        {picking ? (
          ...
          <button onClick={openPicker} style={{
            background:'none', border:'none', cursor:'pointer',
            fontFamily:SANS, fontSize:17, fontWeight:600, color:COLORS.ink,
            display:'flex', alignItems:'center', gap:4,
          }}>
            ...
            {cells.map((d, i) => {
              ...
              return (
                <button key={i} onClick={() => tap(d)} style={{
                  ...
                  fontFamily:SANS, fontSize:17, fontWeight:(isStart||isEnd) ? 600 : 400,
                }}>{d}</button>
              );
            })}
          </div>
...
const CITY_DB = [
  ...
  { key:'uk',          kor:'영국',       eng:'United Kingdom', flag:'🇬🇧', zone:'Europe/London',        currency:'USD', lat:51.5074,  lon:-0.1278   },
  { key:'thailand',    kor:'태국',       eng:'Thailand',       flag:'🇹🇭', zone:'Asia/Bangkok',         currency:'USD', lat:13.7563,  lon:100.5018  },
  { key:'indonesia',   kor:'인도네시아', eng:'Indonesia',      flag:'🇮🇩', zone:'Asia/Makassar',        currency:'USD', lat:-8.3405,  lon:115.0920  },
  { key:'singapore',   kor:'싱가포르',   eng:'Singapore',      flag:'🇸🇬', zone:'Asia/Singapore',       currency:'USD', lat:1.3521,   lon:103.8198  },
  ...
  { key:'czechia',     kor:'체코',       eng:'Czech Republic', flag:'🇨🇿', zone:'Europe/Prague',        currency:'USD', lat:50.0755,  lon:14.4378   },
  { key:'australia',   kor:'호주',       eng:'Australia',      flag:'🇦🇺', zone:'Australia/Sydney',     currency:'USD', lat:-33.8688, lon:151.2093  },
  { key:'uae',         kor:'아랍에미리트',eng:'UAE',            flag:'🇦🇪', zone:'Asia/Dubai',           currency:'USD', lat:25.2048,  lon:55.2708   },
  { key:'turkey',      kor:'터키',       eng:'Turkey',         flag:'🇹🇷', zone:'Europe/Istanbul',      currency:'USD', lat:41.0082,  lon:28.9784   },
  { key:'vietnam',     kor:'베트남',     eng:'Vietnam',        flag:'🇻🇳', zone:'Asia/Ho_Chi_Minh',    currency:'USD', lat:21.0285,  lon:105.8542  },
  { key:'taiwan',      kor:'대만',       eng:'Taiwan',         flag:'🇹🇼', zone:'Asia/Taipei',          currency:'USD', lat:25.0330,  lon:121.5654  },
  { key:'hongkong',    kor:'홍콩',       eng:'Hong Kong',      flag:'🇭🇰', zone:'Asia/Hong_Kong',       currency:'USD', lat:22.3193,  lon:114.1694  },
  ...
  { key:'malaysia',    kor:'말레이시아', eng:'Malaysia',       flag:'🇲🇾', zone:'Asia/Kuala_Lumpur',    currency:'USD', lat:3.1390,   lon:101.6869  },
  { key:'mexico',      kor:'멕시코',     eng:'Mexico',         flag:'🇲🇽', zone:'America/Mexico_City',  currency:'USD', lat:19.4326,  lon:-99.1332  },
  { key:'canada',      kor:'캐나다',     eng:'Canada',         flag:'🇨🇦', zone:'America/Toronto',      currency:'USD', lat:43.6532,  lon:-79.3832  },
  ...
  { key:'switzerland', kor:'스위스',     eng:'Switzerland',    flag:'🇨🇭', zone:'Europe/Zurich',        currency:'USD', lat:47.3769,  lon:8.5417    },
  ...
  { key:'cambodia',    kor:'캄보디아',   eng:'Cambodia',       flag:'🇰🇭', zone:'Asia/Phnom_Penh',      currency:'USD', lat:11.5564,  lon:104.9282  },
  { key:'philippines', kor:'필리핀',     eng:'Philippines',    flag:'🇵🇭', zone:'Asia/Manila',          currency:'PHP', lat:14.5995,  lon:120.9842  },
  { key:'newzealand',  kor:'뉴질랜드',   eng:'New Zealand',    flag:'🇳🇿', zone:'Pacific/Auckland',     currency:'USD', lat:-36.8485, lon:174.7633  },
  { key:'morocco',     kor:'모로코',     eng:'Morocco',        flag:'🇲🇦', zone:'Africa/Casablanca',    currency:'USD', lat:33.5731,  lon:-7.5898   },
  { key:'peru',        kor:'페루',       eng:'Peru',           flag:'🇵🇪', zone:'America/Lima',         currency:'USD', lat:-12.0464, lon:-77.0428  },
  { key:'hawaii',      kor:'하와이',     eng:'Hawaii',         flag:'🇺🇸', zone:'Pacific/Honolulu',     currency:'USD', lat:21.3069,  lon:-157.8583 },
  { key:'korea',       kor:'한국',       eng:'Korea',          flag:'🇰🇷', zone:'Asia/Seoul',           currency:'USD', lat:37.5665,  lon:126.9780  },
  { key:'china',       kor:'중국',       eng:'China',          flag:'🇨🇳', zone:'Asia/Shanghai',        currency:'USD', lat:39.9042,  lon:116.4074  },
  { key:'india',       kor:'인도',       eng:'India',          flag:'🇮🇳', zone:'Asia/Kolkata',         currency:'USD', lat:28.6139,  lon:77.2090   },
  { key:'brazil',      kor:'브라질',     eng:'Brazil',         flag:'🇧🇷', zone:'America/Sao_Paulo',    currency:'USD', lat:-23.5505, lon:-46.6333  },
  // 유럽
  { key:'norway',      kor:'노르웨이',   eng:'Norway',         flag:'🇳🇴', zone:'Europe/Oslo',          currency:'USD', lat:59.9139,  lon:10.7522   },
  { key:'sweden',      kor:'스웨덴',     eng:'Sweden',         flag:'🇸🇪', zone:'Europe/Stockholm',     currency:'USD', lat:59.3293,  lon:18.0686   },
  { key:'denmark',     kor:'덴마크',     eng:'Denmark',        flag:'🇩🇰', zone:'Europe/Copenhagen',    currency:'USD', lat:55.6761,  lon:12.5683   },
  { key:'finland',     kor:'핀란드',     eng:'Finland',        flag:'🇫🇮', zone:'Europe/Helsinki',      currency:'EUR', lat:60.1699,  lon:24.9384   },
  { key:'iceland',     kor:'아이슬란드', eng:'Iceland',        flag:'🇮🇸', zone:'Atlantic/Reykjavik',   currency:'USD', lat:64.1265,  lon:-21.8174  },
  ...
  { key:'poland',      kor:'폴란드',     eng:'Poland',         flag:'🇵🇱', zone:'Europe/Warsaw',        currency:'USD', lat:52.2297,  lon:21.0122   },
  { key:'hungary',     kor:'헝가리',     eng:'Hungary',        flag:'🇭🇺', zone:'Europe/Budapest',      currency:'USD', lat:47.4979,  lon:19.0402   },
  ...
  { key:'romania',     kor:'루마니아',   eng:'Romania',        flag:'🇷🇴', zone:'Europe/Bucharest',     currency:'USD', lat:44.4268,  lon:26.1025   },
  { key:'bulgaria',    kor:'불가리아',   eng:'Bulgaria',       flag:'🇧🇬', zone:'Europe/Sofia',         currency:'USD', lat:42.6977,  lon:23.3219   },
  { key:'serbia',      kor:'세르비아',   eng:'Serbia',         flag:'🇷🇸', zone:'Europe/Belgrade',      currency:'USD', lat:44.8176,  lon:20.4633   },
  { key:'bosnia',      kor:'보스니아',   eng:'Bosnia',         flag:'🇧🇦', zone:'Europe/Sarajevo',      currency:'USD', lat:43.8563,  lon:18.4131   },
  { key:'montenegro',  kor:'몬테네그로', eng:'Montenegro',     flag:'🇲🇪', zone:'Europe/Podgorica',     currency:'EUR', lat:42.4304,  lon:19.2594   },
  { key:'albania',     kor:'알바니아',   eng:'Albania',        flag:'🇦🇱', zone:'Europe/Tirane',        currency:'USD', lat:41.3275,  lon:19.8187   },
  { key:'northmacedonia', kor:'북마케도니아', eng:'North Macedonia', flag:'🇲🇰', zone:'Europe/Skopje', currency:'USD', lat:41.9981, lon:21.4254   },
  ...
  { key:'russia',      kor:'러시아',     eng:'Russia',         flag:'🇷🇺', zone:'Europe/Moscow',        currency:'USD', lat:55.7558,  lon:37.6173   },
  { key:'ukraine',     kor:'우크라이나', eng:'Ukraine',        flag:'🇺🇦', zone:'Europe/Kiev',          currency:'USD', lat:50.4501,  lon:30.5234   },
  ...
  { key:'georgia',     kor:'조지아',     eng:'Georgia',        flag:'🇬🇪', zone:'Asia/Tbilisi',         currency:'USD', lat:41.6938,  lon:44.8015   },
  { key:'armenia',     kor:'아르메니아', eng:'Armenia',        flag:'🇦🇲', zone:'Asia/Yerevan',         currency:'USD', lat:40.1872,  lon:44.5152   },
  { key:'azerbaijan',  kor:'아제르바이잔',eng:'Azerbaijan',    flag:'🇦🇿', zone:'Asia/Baku',            currency:'USD', lat:40.4093,  lon:49.8671   },
  // 중동
  { key:'israel',      kor:'이스라엘',   eng:'Israel',         flag:'🇮🇱', zone:'Asia/Jerusalem',       currency:'USD', lat:31.7683,  lon:35.2137   },
  { key:'jordan',      kor:'요르단',     eng:'Jordan',         flag:'🇯🇴', zone:'Asia/Amman',           currency:'USD', lat:31.9539,  lon:35.9106   },
  { key:'lebanon',     kor:'레바논',     eng:'Lebanon',        flag:'🇱🇧', zone:'Asia/Beirut',          currency:'USD', lat:33.8886,  lon:35.4955   },
  { key:'saudi',       kor:'사우디아라비아', eng:'Saudi Arabia', flag:'🇸🇦', zone:'Asia/Riyadh',       currency:'USD', lat:24.7136,  lon:46.6753   },
  { key:'qatar',       kor:'카타르',     eng:'Qatar',          flag:'🇶🇦', zone:'Asia/Qatar',           currency:'USD', lat:25.2854,  lon:51.5310   },
  { key:'kuwait',      kor:'쿠웨이트',   eng:'Kuwait',         flag:'🇰🇼', zone:'Asia/Kuwait',          currency:'USD', lat:29.3759,  lon:47.9774   },
  { key:'bahrain',     kor:'바레인',     eng:'Bahrain',        flag:'🇧🇭', zone:'Asia/Bahrain',         currency:'USD', lat:26.0667,  lon:50.5577   },
  { key:'oman',        kor:'오만',       eng:'Oman',           flag:'🇴🇲', zone:'Asia/Muscat',          currency:'USD', lat:23.5880,  lon:58.3829   },
  { key:'iran',        kor:'이란',       eng:'Iran',           flag:'🇮🇷', zone:'Asia/Tehran',          currency:'USD', lat:35.6892,  lon:51.3890   },
  // 아시아
  { key:'myanmar',     kor:'미얀마',     eng:'Myanmar',        flag:'🇲🇲', zone:'Asia/Rangoon',         currency:'USD', lat:16.8661,  lon:96.1951   },
  { key:'laos',        kor:'라오스',     eng:'Laos',           flag:'🇱🇦', zone:'Asia/Vientiane',       currency:'USD', lat:17.9757,  lon:102.6331  },
  { key:'mongolia',    kor:'몽골',       eng:'Mongolia',       flag:'🇲🇳', zone:'Asia/Ulaanbaatar',     currency:'USD', lat:47.8864,  lon:106.9057  },
  { key:'nepal',       kor:'네팔',       eng:'Nepal',          flag:'🇳🇵', zone:'Asia/Kathmandu',       currency:'USD', lat:27.7172,  lon:85.3240   },
  { key:'srilanka',    kor:'스리랑카',   eng:'Sri Lanka',      flag:'🇱🇰', zone:'Asia/Colombo',         currency:'USD', lat:6.9271,   lon:79.8612   },
  { key:'bangladesh',  kor:'방글라데시', eng:'Bangladesh',     flag:'🇧🇩', zone:'Asia/Dhaka',           currency:'USD', lat:23.8103,  lon:90.4125   },
  { key:'pakistan',    kor:'파키스탄',   eng:'Pakistan',       flag:'🇵🇰', zone:'Asia/Karachi',         currency:'USD', lat:33.6844,  lon:73.0479   },
  { key:'bhutan',      kor:'부탄',       eng:'Bhutan',         flag:'🇧🇹', zone:'Asia/Thimphu',         currency:'USD', lat:27.4712,  lon:89.6339   },
  { key:'macau',       kor:'마카오',     eng:'Macau',          flag:'🇲🇴', zone:'Asia/Macau',           currency:'USD', lat:22.1987,  lon:113.5439  },
  { key:'kazakhstan',  kor:'카자흐스탄', eng:'Kazakhstan',     flag:'🇰🇿', zone:'Asia/Almaty',          currency:'USD', lat:51.1801,  lon:71.4460   },
  { key:'uzbekistan',  kor:'우즈베키스탄',eng:'Uzbekistan',   flag:'🇺🇿', zone:'Asia/Tashkent',        currency:'USD', lat:41.2995,  lon:69.2401   },
  { key:'kyrgyzstan',  kor:'키르기스스탄',eng:'Kyrgyzstan',   flag:'🇰🇬', zone:'Asia/Bishkek',         currency:'USD', lat:42.8746,  lon:74.5698   },
  // 아프리카
  { key:'egypt',       kor:'이집트',     eng:'Egypt',          flag:'🇪🇬', zone:'Africa/Cairo',         currency:'USD', lat:30.0444,  lon:31.2357   },
  { key:'southafrica', kor:'남아프리카공화국', eng:'South Africa', flag:'🇿🇦', zone:'Africa/Johannesburg', currency:'USD', lat:-25.7479, lon:28.2293  },
  { key:'kenya',       kor:'케냐',       eng:'Kenya',          flag:'🇰🇪', zone:'Africa/Nairobi',       currency:'USD', lat:-1.2921,  lon:36.8219   },
  { key:'tanzania',    kor:'탄자니아',   eng:'Tanzania',       flag:'🇹🇿', zone:'Africa/Dar_es_Salaam', currency:'USD', lat:-6.7924,  lon:39.2083   },
  { key:'ethiopia',    kor:'에티오피아', eng:'Ethiopia',       flag:'🇪🇹', zone:'Africa/Addis_Ababa',   currency:'USD', lat:9.0320,   lon:38.7469   },
  { key:'tunisia',     kor:'튀니지',     eng:'Tunisia',        flag:'🇹🇳', zone:'Africa/Tunis',         currency:'USD', lat:36.8065,  lon:10.1815   },
  { key:'ghana',       kor:'가나',       eng:'Ghana',          flag:'🇬🇭', zone:'Africa/Accra',         currency:'USD', lat:5.6037,   lon:-0.1870   },
  { key:'nigeria',     kor:'나이지리아', eng:'Nigeria',        flag:'🇳🇬', zone:'Africa/Lagos',         currency:'USD', lat:6.5244,   lon:3.3792    },
  { key:'senegal',     kor:'세네갈',     eng:'Senegal',        flag:'🇸🇳', zone:'Africa/Dakar',         currency:'USD', lat:14.7167,  lon:-17.4677  },
  { key:'mauritius',   kor:'모리셔스',   eng:'Mauritius',      flag:'🇲🇺', zone:'Indian/Mauritius',     currency:'USD', lat:-20.1609, lon:57.4977   },
  { key:'seychelles',  kor:'세이셸',     eng:'Seychelles',     flag:'🇸🇨', zone:'Indian/Mahe',          currency:'USD', lat:-4.6796,  lon:55.4920   },
  { key:'madagascar',  kor:'마다가스카르',eng:'Madagascar',    flag:'🇲🇬', zone:'Indian/Antananarivo',  currency:'USD', lat:-18.9137, lon:47.5361   },
  { key:'zimbabwe',    kor:'짐바브웨',   eng:'Zimbabwe',       flag:'🇿🇼', zone:'Africa/Harare',        currency:'USD', lat:-17.8252, lon:31.0335   },
  { key:'zambia',      kor:'잠비아',     eng:'Zambia',         flag:'🇿🇲', zone:'Africa/Lusaka',        currency:'USD', lat:-15.4167, lon:28.2833   },
  { key:'algeria',     kor:'알제리',     eng:'Algeria',        flag:'🇩🇿', zone:'Africa/Algiers',       currency:'USD', lat:36.7372,  lon:3.0865    },
  { key:'uganda',      kor:'우간다',     eng:'Uganda',         flag:'🇺🇬', zone:'Africa/Kampala',       currency:'USD', lat:0.3476,   lon:32.5825   },
  // 아메리카
  { key:'argentina',   kor:'아르헨티나', eng:'Argentina',      flag:'🇦🇷', zone:'America/Argentina/Buenos_Aires', currency:'USD', lat:-34.6037, lon:-58.3816 },
  { key:'chile',       kor:'칠레',       eng:'Chile',          flag:'🇨🇱', zone:'America/Santiago',     currency:'USD', lat:-33.4489, lon:-70.6693  },
  { key:'colombia',    kor:'콜롬비아',   eng:'Colombia',       flag:'🇨🇴', zone:'America/Bogota',       currency:'USD', lat:4.7110,   lon:-74.0721  },
  { key:'ecuador',     kor:'에콰도르',   eng:'Ecuador',        flag:'🇪🇨', zone:'America/Guayaquil',    currency:'USD', lat:-0.1807,  lon:-78.4678  },
  { key:'bolivia',     kor:'볼리비아',   eng:'Bolivia',        flag:'🇧🇴', zone:'America/La_Paz',       currency:'USD', lat:-16.5000, lon:-68.1500  },
  { key:'uruguay',     kor:'우루과이',   eng:'Uruguay',        flag:'🇺🇾', zone:'America/Montevideo',   currency:'USD', lat:-34.9011, lon:-56.1645  },
  { key:'paraguay',    kor:'파라과이',   eng:'Paraguay',       flag:'🇵🇾', zone:'America/Asuncion',     currency:'USD', lat:-25.2637, lon:-57.5759  },
  { key:'venezuela',   kor:'베네수엘라', eng:'Venezuela',      flag:'🇻🇪', zone:'America/Caracas',      currency:'USD', lat:10.4806,  lon:-66.9036  },
  { key:'cuba',        kor:'쿠바',       eng:'Cuba',           flag:'🇨🇺', zone:'America/Havana',       currency:'USD', lat:23.1136,  lon:-82.3666  },
  { key:'dominican',   kor:'도미니카공화국', eng:'Dominican Republic', flag:'🇩🇴', zone:'America/Santo_Domingo', currency:'USD', lat:18.4861, lon:-69.9312 },
  { key:'jamaica',     kor:'자메이카',   eng:'Jamaica',        flag:'🇯🇲', zone:'America/Jamaica',      currency:'USD', lat:17.9988,  lon:-76.7932  },
  { key:'costarica',   kor:'코스타리카', eng:'Costa Rica',     flag:'🇨🇷', zone:'America/Costa_Rica',   currency:'USD', lat:9.9281,   lon:-84.0907  },
  { key:'panama',      kor:'파나마',     eng:'Panama',         flag:'🇵🇦', zone:'America/Panama',       currency:'USD', lat:8.9943,   lon:-79.5188  },
  { key:'guatemala',   kor:'과테말라',   eng:'Guatemala',      flag:'🇬🇹', zone:'America/Guatemala',    currency:'USD', lat:14.6349,  lon:-90.5069  },
  // 오세아니아
  { key:'fiji',        kor:'피지',       eng:'Fiji',           flag:'🇫🇯', zone:'Pacific/Fiji',         currency:'USD', lat:-18.1248, lon:178.4501  },
  { key:'samoa',       kor:'사모아',     eng:'Samoa',          flag:'🇼🇸', zone:'Pacific/Apia',         currency:'USD', lat:-13.8506, lon:-171.7513 },
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'14px 20px' }}>
                ...
                <div style={{ position:'relative', borderRadius:14, background:COLORS.card, border:`1.5px solid ${COLORS.line}` }}>
                  ...
                  {ghostSuffix && (
                    <div aria-hidden="true" style={{
                      ...
                      fontFamily:SANS, fontSize:17, lineHeight:'normal',
                      borderRadius:14,
                    }}>
          ...
          {step === 2 && (() => {
            ...
            return (
              <div>
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          <input value={city} autoFocus={i===0 && !cityDrag}
                            ...
                            style={{ width:'100%', boxSizing:'border-box', padding:'12px 40px 12px 16px', border:'none', borderRadius:14, outline:'none', background:'transparent', fontFamily:SANS, fontSize:17, color:COLORS.ink, position:'relative', zIndex:1 }}
                          />
                          {cityGhostSuffix && (
                ...
                <button onMouseDown={e => e.preventDefault()} onClick={() => setCities(prev=>[...prev,''])} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', color:COLORS.mute, fontFamily:SANS, fontSize:17, padding:'6px 0', marginTop:4 }}>
          ...
          {step === 3 && (
            <div>
              {startIso && endIso && dayCount > 0 && (
                <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.accent, marginBottom:12, textAlign:'center', fontWeight:600 }}>
            ...
            const makeAirportInput = (value, setValue, label, isAutoFocus, placeholder) => {
              ...
              return (
                <div style={{ marginBottom:18 }}>
                  <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginBottom:6, textTransform:'uppercase', letterSpacing:'0.05em' }}>{label}</div>
                  <div style={{ position:'relative', borderRadius:14, background:COLORS.card, border:`1.5px solid ${COLORS.line}` }}>
                    {ghostSuffix && (
                      <div aria-hidden="true" style={{ position:'absolute', inset:0, padding:'12px 40px 12px 16px', display:'flex', alignItems:'center', pointerEvents:'none', overflow:'hidden', fontFamily:SANS, fontSize:17, lineHeight:'normal', borderRadius:14 }}>
                  ...
                  {codeMatch && (
                    <div onMouseDown={e => { e.preventDefault(); setValue(lang === 'ko' ? codeMatch.kor : codeMatch.eng); }}
                      ...
                      <span style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute }}>{codeMatch.code}</span>
              ...
              <div>
                ...
                {!arrAirport && citySuggests.length > 0 && (
                  <div style={{ marginBottom:18, marginTop:-8 }}>
                    <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginBottom:8, textTransform:'uppercase', letterSpacing:'0.05em' }}>
                      ...
                      {citySuggests.map(apt => {
                        ...
                        return (
                          <button key={apt} onMouseDown={e=>e.preventDefault()} onClick={() => setArrAirport(aptName)}
                            style={{ display:'flex', alignItems:'center', gap:6, background:COLORS.card, border:`1.5px solid ${COLORS.line}`, borderRadius:20, padding:'6px 14px 6px 10px', cursor:'pointer', fontFamily:SANS, fontSize:16.5, color:COLORS.ink }}>
                            <span style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:0.5 }}>{info?.code}</span>
                            <span>{aptName}</span>
                          </button>
                        );
                      })}
                ...
                <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, lineHeight:1.5 }}>
                  {t('wizNoFlight')}
                </div>
              </div>
            );
            ...
            <div>
              {!skipHotel && hotels.map((h, i) => (
                <div key={i} style={{ background:COLORS.card, borderRadius:14, padding:'12px 14px', marginBottom:10, border:`1px solid ${COLORS.line}` }}>
                  ...
                  />
                  {dayCount > 1 && (
                    <div style={{ display:'flex', alignItems:'center', gap:8, fontFamily:SANS, fontSize:16, color:COLORS.mute }}>
                      <span>Day</span>
                      <input type="number" min={1} max={dayCount} value={h.from}
                        ...
                        style={{ width:40, textAlign:'center', border:`1px solid ${COLORS.line}`, borderRadius:6, padding:'3px 4px', fontFamily:SANS, fontSize:16, outline:'none' }}
                      />
                      <span>~</span>
              ...
              {!skipHotel && (
                <button onClick={() => setHotels(prev=>[...prev,{name:'',from:1,to:dayCount||1}])} style={{ display:'flex', alignItems:'center', gap:6, background:'none', border:'none', cursor:'pointer', color:COLORS.mute, fontFamily:SANS, fontSize:17, padding:'4px 0', marginBottom:14 }}>
                  <span style={{ fontSize:18 }}>+</span> {t('wizAddHotel')}
                </button>
              )}
              <button onClick={() => setSkipHotel(s=>!s)} style={{ width:'100%', padding:'10px', border:`1px solid ${skipHotel?COLORS.accent:COLORS.line}`, borderRadius:12, background:'transparent', fontFamily:SANS, fontSize:17, color:skipHotel?COLORS.accent:COLORS.mute, cursor:'pointer' }}>
          ...
          {step === HP_STEP && (() => {
            ...
            return (
              <div>
                {loading && (
                  <div style={{ textAlign:'center', padding:'48px 0', fontFamily:SANS, fontSize:17, color:COLORS.mute }}>
                    {t('wizLoadingPlaces')}
                  </div>
                )}
                {!loading && placeErr && (
                  <div style={{ textAlign:'center', padding:'24px 0', fontFamily:SANS, fontSize:16, color:'#E03C31' }}>
                    {placeErr}
                  </div>
                )}
                ...
                {!loading && cityPlaces.length > 0 && (
                  <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginBottom:12 }}>
                    ...
                    return (
                      <button key={p.id} onClick={() => togglePlace(p.id)} style={{
                        ...
                        <div style={{ height:88, position:'relative' }}>
                          ...
                          {p.type === 'hiking_trail' && (
                            <div style={{ position:'absolute', bottom:6, left:7, background:'rgba(0,0,0,0.52)', borderRadius:5, padding:'2px 6px', fontFamily:SANS, fontSize:16, color:'#fff', letterSpacing:0.3 }}>
                              {t('wizHiking')}
                            </div>
                          )}
                          {sel && (
                        ...
                        <div style={{ padding:'7px 9px 9px' }}>
                          <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.ink, lineHeight:1.35, fontWeight: sel ? 600 : 400 }}>{p.name}</div>
                          {p.nameOrig && <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:2, lineHeight:1.2 }}>{p.nameOrig}</div>}
                        </div>
                      </button>
                    );
                  })}
                ...
                {!loading && hiddenCnt > 0 && (
                  <button onMouseDown={e=>e.preventDefault()} onClick={() => setShowCount(c => c + STEP)}
                    style={{ marginTop:14, width:'100%', padding:'11px 0', borderRadius:12, border:`1.5px solid ${COLORS.line}`, background:'transparent', fontFamily:SANS, fontSize:17, color:COLORS.mute, cursor:'pointer' }}>
        ...
        <div style={{ padding:'10px 20px 20px', display:'flex', gap:8, flexShrink:0, borderTop:`1px solid ${COLORS.line}` }}>
          {(step > 1 || (step === HP_STEP && cityStep > 0))
            ? <button onMouseDown={e=>e.preventDefault()} onClick={handleBack} style={{ padding:'11px 18px', borderRadius:12, border:`1px solid ${COLORS.line}`, background:'transparent', fontFamily:SANS, fontSize:17, color:COLORS.ink, cursor:'pointer' }}>{t('wizPrev')}</button>
            : <button onClick={onClose} style={{ padding:'11px 18px', borderRadius:12, border:`1px solid ${COLORS.line}`, background:'transparent', fontFamily:SANS, fontSize:17, color:COLORS.mute, cursor:'pointer' }}>{t('cancel')}</button>
          }
    ...
    <div style={{ position:'fixed', inset:0, zIndex:210, background:'rgba(0,0,0,0.4)',
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:'6px 20px 16px', borderBottom:`1px solid ${COLORS.line}` }}>
          ...
          <div style={{ fontFamily:SANS, fontSize:16.5, color:COLORS.mute, marginTop:4 }}>
            이메일로 동행인을 추가해요
          </div>
        </div>
        ...
        {pendingInvites.length > 0 && (
          <div style={{ margin:'14px 16px 0', background:'#FFF8E1', borderRadius:14, padding:14 }}>
            <div style={{ fontFamily:MONO, fontSize:16, color:'#B8860B', letterSpacing:'0.1em', marginBottom:8 }}>
              ...
              <div key={inv.id} style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                {inv.fromPhoto
                  ...
                  : <div style={{ width:34, height:34, borderRadius:17, background:COLORS.accent,
                      display:'flex', alignItems:'center', justifyContent:'center',
                      fontFamily:SANS, fontSize:17, color:'#fff' }}>{(inv.fromName||'?')[0]}</div>
                }
                <div style={{ flex:1, minWidth:0 }}>
                ...
                <button onClick={() => handleAccept(inv)} disabled={!!accepting} style={{
                  ...
                  background: accepting === inv.id ? COLORS.mute : COLORS.ink, color:COLORS.bg, fontFamily:SANS, fontSize:16, fontWeight:500,
                ...
                <button onClick={() => fbRejectInvite(inv.id).then(() => setPendingInvites(p=>p.filter(i=>i.id!==inv.id)))} style={{
                  border:`1px solid ${COLORS.line}`, borderRadius:9, padding:'6px 10px', cursor:'pointer',
                  background:'transparent', fontFamily:SANS, fontSize:16, color:COLORS.mute,
                }}>거절</button>
              </div>
            ))}
          </div>
        ...
        <div style={{ padding:'16px 16px 0' }}>
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', marginBottom:8 }}>
            여행 선택 (선택사항)
          </div>
          ...
          <div style={{ marginTop:16, fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', marginBottom:8 }}>
            이메일로 초대
          </div>
          <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginBottom:10 }}>
          ...
          <input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)}
            ...
            style={{ width:'100%', padding:'12px 14px', borderRadius:12,
              ...
              fontFamily:SANS, fontSize:17, color:COLORS.ink, boxSizing:'border-box', outline:'none' }}/>
          {inviteMsg && (
            <div style={{ marginTop:8, fontFamily:SANS, fontSize:16,
  ...
  return (
    <div>
      <div onClick={() => setLang(code)} style={{
        ...
        <div style={{ flex:1, fontFamily:SANS, fontSize:17, fontWeight:500, color:COLORS.ink }}>{label}</div>
    ...
    <div style={{ position:'fixed', inset:0, zIndex:210, background:'rgba(0,0,0,0.4)',
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ overflowY:'auto', flex:1, padding:'12px 16px' }}>
          {/* Appearance */}
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6, paddingLeft:4 }}>{t('appearance')}</div>
          ...
          {/* Language */}
          <div style={{ fontFamily:MONO, fontSize:16, color:COLORS.mute, letterSpacing:'0.1em', textTransform:'uppercase', marginBottom:6, paddingLeft:4 }}>{t('language')}</div>
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding:'12px 20px 16px', display:'flex', gap:14, alignItems:'center',
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:2 }}>{authUser.email}</div>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            ...
            <button onClick={() => { fbSignOut(); onClose(); }} style={{
              ...
              background:'transparent', fontFamily:SANS, fontSize:16, color:COLORS.mute, cursor:'pointer',
            }}>{t('logout')}</button>
          </div>
        </div>
        <div style={{ overflowY:'auto', flex:1, padding:'14px 16px' }}>
          ...
          <div style={{
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontFamily:SANS, fontSize:16, color:COLORS.mute, marginTop:2 }}>{t('companionsDesc')}</div>
            </div>
            <div style={{ display:'flex', gap:8, flexShrink:0 }}>
              ...
              <button onClick={() => { onClose(); setTimeout(() => onAddCompanion(null), 100); }} style={{
                ...
                fontFamily:SANS, fontSize:16, fontWeight:600, color:'#fff',
              }}>{t('addComp')}</button>
            </div>
          </div>
          ...
          {!confirmDelete ? (
            <button onClick={() => setConfirmDelete(true)} style={{
              ...
              fontFamily:SANS, fontSize:16, color:COLORS.mute,
              textAlign:'center',
            ...
            <div style={{
              ...
              <div style={{ fontFamily:SANS, fontSize:16, color:'#B71C1C', lineHeight:1.5, marginBottom:14 }}>
                {t('confirmDeleteDesc')}
              </div>
              {deleteError && (
                <div style={{ fontFamily:SANS, fontSize:16, color:'#C62828', marginBottom:10 }}>{t('deleteError')}</div>
              )}
              <div style={{ display:'flex', gap:8 }}>
  ...
  render() {
    if (this.state.err) {
      return React.createElement('div', {
        ...
        React.createElement('div', { style:{ fontFamily:'sans-serif', fontSize:17, color:'#666', marginBottom:28 } }, '새로고침하면 대부분 해결됩니다'),
        React.createElement('button', {
          onClick: () => window.location.reload(),
          style: { padding:'12px 28px', borderRadius:14, border:'none', background:'#1a1a1a',
            color:'#fff', fontFamily:'sans-serif', fontSize:17, fontWeight:600, cursor:'pointer' }
        }, '새로고침')
      );
    }
  ...
  if (!activeTripId) appBody = (
    ...
    <div style={{ minHeight:'100vh', background:'#1a1a1a', display:'flex', flexDirection:'column',
      ...
      <button onClick={async () => {
        ...
        color:'#fff', fontFamily:SANS, fontSize:17, fontWeight:700, cursor:'pointer',
      ...
      {saveConfirm && ReactDOM.createPortal(
        <div style={{ position:'fixed', inset:0, zIndex:700,
          ...
          <div style={{ background:COLORS.bg, borderRadius:22, padding:'28px 24px 20px',
            ...
            <div style={{ fontFamily:SANS, fontSize:17, color:COLORS.mute, marginBottom:24, lineHeight:1.5 }}>
              변경된 내용이 저장됩니다.
            </div>
